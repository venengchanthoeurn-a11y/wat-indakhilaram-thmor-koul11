<?php
include 'config.php';
requireLogin(); // Ensure user is logged in, assumes $_SESSION['user_id'] is set
...
// Fetch monks with their kuti
$monks = $pdo->query("SELECT m.*, k.kuti_name FROM monks m LEFT JOIN kuti k ON m.kuti_id = k.id ORDER BY k.kuti_name ASC, m.khmer_name ASC")->fetchAll();
?>
...
<head>
    ...
    <title>មើលព្រះសង្ឃតាមកុដិ - កំណត់ត្រាព្រះពុទ្ធសាសនា</title>
...
<body>
    ...
    <div class="container">
        <h3><i class="bi bi-people-fill me-2"></i>មើលព្រះសង្ឃតាមកុដិ</h3>
        
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0"><i class="bi bi-list-check me-2"></i>បញ្ជីព្រះសង្ឃតាមកុដិ</h5>
            </div>
                    ...
                    <table class="table table-striped table-hover mb-0">
                        <thead>
                            <tr>
                                ...
                                <th data-label="ថ្ងៃចូល"><i class="bi bi-calendar-event me-1"></i>ថ្ងៃចូល</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php if (empty($monks)): ?>
                                <tr>
                                    <td colspan="3" class="no-data">
                                        <i class="bi bi-inbox me-2"></i>មិនមានព្រះសង្ឃនៅក្នុងបញ្ជីទេ។
                                    </td>
                                </tr>
                            <?php else: ?>
                                <?php foreach ($monks as $monk): ?>
                                    <tr>
                                        ...
                                        <td data-label="កុដិ"><?php echo htmlspecialchars($monk['kuti_name'] ?? 'មិនបានកំណត់'); ?></td>
                                        <td data-label="ថ្ងៃចូល"><?php echo date('d-m-Y', strtotime($monk['created_at'])); ?></td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        </tbody>
        ...
        <div class="d-flex justify-content-end mt-4">
            ...
            <button class="btn btn-print-all ms-2" onclick="logAction('Print'); window.print()">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Function to log action via AJAX to log_action.php
            ...
            // Enhance print functionality for professionalism (optional)
            window.onbeforeprint = () => {
                // Optionally hide elements not needed for print (like buttons)
                document.querySelectorAll('.d-flex.justify-content-end').forEach(el => el.style.display = 'none');
            };
            window.onafterprint = () => {
                // Restore elements after printing
                document.querySelectorAll('.d-flex.justify-content-end').forEach(el => el.style.display = 'flex');
            };
        });
    </script>
</body>
